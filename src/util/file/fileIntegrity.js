...
export async function scanFile(file, options = {}) {
  ...
  try {
    ...
    if (stats.size > maxFileSize) {
      ...
      throw new APIError(`File too large. Maximum size: ${(maxFileSize / (1024 * 1024)).toFixed(2)}MB`, 400);
    }
    ...
    if (!allowedTypes[file.mimetype]) {
      ...
      throw new APIError(`File type not allowed. Allowed types: ${Object.keys(allowedTypes).join(", ")}`, 400);
    }
    ...
    if (!extensionValid) {
      ...
      throw new APIError(`File extension mismatch. Expected: ${allowedTypes[file.mimetype].join(", ")}`, 400);
    }
    ...
    if (scanSuspiciousPatterns) {
      ...
      if (!Array.isArray(suspiciousPatterns)) {
        throw new APIError("File security scan failed: Unable to scan for suspicious patterns", 500);
      }
